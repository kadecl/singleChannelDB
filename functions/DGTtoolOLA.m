classdef DGTtoolOLA < handle
% ------------Change point---------------------------------------------------------------------------------------------------------------
    % the class name DGTtool -> DGTtoolOLA

    % 279-283       if nargout > 1
    ...
    %               end
    % ->358-361     if nargout > 2
    %                   [X,obj.defInconverter,f,t] = DGT_factorAlg(x,obj.win,obj.shift,obj.FFTnum,obj.factorIdx);
    %               else
    %                   [X,obj.defInconverter] = DGT_factorAlg(x,obj.win,obj.shift,obj.FFTnum,obj.factorIdx,obj.defInconverter);
    %               end
    
    % 325           x = invDGT_factorAlg(X,sWin,obj.shift,obj.FFTnum,obj.factorIdx);
    % ->404              x = invDGT_usualAlg(X,sWin,obj.FFTnum,obj.OLAindex,shift);

    % 331           x = invDGT_usualAlg(X,sWin,obj.FFTnum,obj.OLAindex);
    % ->410         x = invDGT_usualAlg(X,sWin,obj.FFTnum,obj.OLAindex,obj.shift);

    % 1413          function [X,f,t] = DGT_usualAlg(x,win,shift,FFTnum,paddedSiglen)
    % -> 1492       function [Xfull,f,t] = DGT_factorAlg(x,win,shift,FFTnum,in)
              
    % 1439          X = Xfull(1:floor(FFTnum/2)+1,:,:);  
    % ->1519        Xfull=circshift(Xfull/FFTnum,-(FFTnum/shift-1),2);
    
    % 1448          function x = invDGT_usualAlg(X,win,FFTnum,OLAidx)
    % ->1527        function x = invDGT_usualAlg(X,win,FFTnum,OLAidx,shift)

    % 1450
    % ->1530        X=circshift(X*FFTnum,(FFTnum/shift-1),2);

    % 1451          wx = ifft([X; zeros(FFTnum-size(X,1),size(X,2),size(X,3))],'symmetric');
    % ->1533        wx = ifft([X; zeros(FFTnum-size(X,1),size(X,2),size(X,3))]);
     
    % 1482          function [X,f,t] = DGT_factorAlg(x,win,shift,FFTnum,in)
    % ->1563        function [Xfull,defInconverter,f,t] = DGT_factorAlg(x,win,shift,FFTnum,in,defInconverter)

    % 1495          C = ifft(C,[],4,'symmetric')   
    % ->1577        C = ifft(C,[],4);
            
    % 1505          
    % ->1589-1593   if isempty(defInconverter)
    %                   defInconverter=calculateDefInConverter(Xfull,shift,FFTnum);
    %               end
    %               Xfull=Xfull/FFTnum.*defInconverter;

    % 1513          function x = invDGT_factorAlg(X,win,shift,FFTnum,in)
    % ->1602        function x = invDGT_factorAlg(X,win,shift,FFTnum,in,defInconverter)

    % 1523        
    % ->1615        X=X.*conj(defInconverter);

    % 1526          wx = ifft([X; zeros(FFTnum-size(X,1),size(X,2),size(X,3))],'symmetric');
    % ->1621        wx = ifft(X);
    
    % 1531          psi = ifft(psi,[],4,'symmetric');
    % ->1628        psi = ifft(psi,[],4);

    % 1533          x(in.xIdx) = psi;
    % ->1630            x(in.xIdx) = psi*FFTnum;

    % 1587
    % ->1684-1689   function defConverter = calculateDefInConverter(X,shift,FFTnum) 
    %                   f = (0:size(X,1)-1)';
...
%==========================================================================================================================================
    ...
    properties (SetAccess = private)
        ...
        isDual = false      % True when dualWin is dual of winh_inc
    ...
    methods % main
        ...
        function obj = DGTtoolOLA(options)
            ...
            elseif isfield(options,'windowName')
                [obj.win,obj.diffWin] = DGTtoolOLA.getWindow(options.windowLength,options.windowName);
            else
                [obj.win,obj.diffWin] = DGTtoolOLA.getWindow(options.windowLength,'4termC5Nuttall');
            end
        ...
        function [X,f,t] = DGT(obj,x)
            ...
            if useFactorizationAlgorithm
%                 error 'Don't compute Factorization
                if ~isequal(obj.sigLen,size(x,1))
        ...
        function x = H(obj,X,sWin)
            ...
            if useFactorizationAlgorithm
                ...
                x = invDGT_factorAlg(X,sWin,obj.shift,obj.FFTnum,obj.factorIdx,obj.defInconverter);
            else
                if ~isequal(size(obj.OLAindex,1:3),[length(sWin) size(X,[2 3])])
                    computeIndexForOLA(obj,X,sWin);
                end
                
                x = invDGT_usualAlg(X,sWin,obj.FFTnum,obj.OLAindex,obj.shift);
            end
        ...
        function x = pinv(obj,X)
            ...
            if isempty(obj.dualWin) || ~obj.isDual || signalLength < length(obj.dualWin)
                ...
                newLen = DGTtoolOLA.sigLenForFactorDGT(newLen,obj.shift,obj.FFTnum);
        ...
        function [reassignedS,f,t,X,IF,GD,dXdt,dXdf] = reassign(obj,x,epsilon)
            ...
            if isempty(obj.diffWin)
                obj.sigLen = DGTtoolOLA.sigLenForFactorDGT(size(x,1),obj.shift,obj.FFTnum);
                dWin = DGTtoolOLA.computeNumericalDiffWin(nWin,obj.sigLen) * obj.FFTnum / length(obj.win);
                
                x    = DGTtoolOLA.extendSignalByZeroPad(x,obj.sigLen);
                tWin = DGTtoolOLA.extendSignalByZeroPad(tWin,obj.sigLen);
                nWin = DGTtoolOLA.extendSignalByZeroPad(nWin,obj.sigLen);
            else
        ...
        function setWindow(obj,windowLength,windowName,varargin)
            ...
            [obj.win,obj.diffWin] = DGTtoolOLA.getWindow(windowLength,windowName,varargin{:});
        end
        ...
        function makeWindowTight(obj,signalLength)
            ...
            obj.win = DGTtoolOLA.computeCanonicalTight(obj.win,obj.shift,obj.FFTnum,signalLength);
            obj.dualWin = obj.win;
        end
    ...
    methods (Static)
            ...
            if ismember(winName,winList(1:7)) % cosine window case
                ...
                if nargout == 2
                    ...
                    elseif win(1) < eps * length(win)
                        diffWin = DGTtoolOLA.computeNumericalDiffWin(win,2^nextpow2(2*length(win)));
        ...
        function dualWin = computeCanonicalDual(win,shift,FFTnum,sigLen)
            ...
            if FFTnum >= length(win)
                ...
                sigLen = DGTtoolOLA.sigLenForFactorDGT(sigLen,shift,FFTnum);
                [c,~,p,q,d,k,l,r,s] = getConstantsForFacAlg(shift,FFTnum,sigLen/shift);
                idx = getWinIdxForFacAlg(k,l,r,s,c,p,q,d);
                
                win = DGTtoolOLA.extendSignalByZeroPad(win,sigLen);
                phi = fft(win(idx),[],4);
        ...
        function [tf,reconstErrorBound] = isdual(win,dualWin,shift,FFTnum)
            ...
            win     = DGTtoolOLA.zeroPadForFactorDGT(win,    shift,FFTnum,maxLen);
            dualWin = DGTtoolOLA.zeroPadForFactorDGT(dualWin,shift,FFTnum,maxLen);
        ...
        function y = extendSignalByZeroPad(x,outputLength)
            ...
            y = DGTtoolOLA.zeroPad(x,outputLength-size(x,1));
        end
        ...
        function y = zeroPadForFactorDGT(x,shift,FFTnum,minLen)
            ...
            newLength = DGTtoolOLA.sigLenForFactorDGT(minLen,shift,FFTnum);
            y = DGTtoolOLA.extendSignalByZeroPad(x,newLength);
        end
...
%             error 'Don't compute Factorization
        ...
        function x = zeroPadForFactorAlg(obj,x,newLength)
            x = DGTtoolOLA.zeroPadForFactorDGT(x,obj.shift,obj.FFTnum,newLength);
        end
        ...
        function canonicalDual = getCanonicalDualWin(obj,sigLen)
            canonicalDual = DGTtoolOLA.computeCanonicalDual(obj.win,obj.shift,obj.FFTnum,sigLen);
        ...
        function checkDualWin(obj)
            if obj.isNotCompDual
                obj.isDual = DGTtoolOLA.isdual(obj.win,obj.dualWin,obj.shift,obj.FFTnum);
        ...
        function setCanonicalFlag(obj)
            if obj.isDual
                canonicalDual = DGTtoolOLA.computeCanonicalDual(obj.win,obj.shift,obj.FFTnum,length(obj.dualWin));
...
function [Xfull,f,t] = DGT_usualAlg(x,win,shift,FFTnum,paddedSiglen)
...
Idx=reshape((1:numel(x)),size(x,1),size(x,2));
...
% end
if size(x,2)==1
    xIdx = (buffer([Idx(end); Idx(1:end-1)],winLen,overlap,buffer(Idx(end-rotNum:end-1),overlap)));
    wx = win .* x(xIdx);
else
    for n = 1:size(x,2)
        xIdx(:,:,n) = mod(buffer(Idx(:,n),winLen,overlap,buffer(Idx(end-rotNum+1:end,n),overlap)) ...
            +(overlap-1),Idx(end,1))+(Idx(end,1)*(n-1))+1;
    end
    wx = win .* x(xIdx)/FFTnum;
end
...
function x = invDGT_usualAlg(X,win,FFTnum,OLAidx,shift)
% Over-lap add (OLA) is performed by accumarray and OLAidx.

X=circshift(X*FFTnum,(FFTnum/shift-1),2);

% wx = ifft([X; zeros(FFTnum-size(X,1),size(X,2),size(X,3))],'symmetric');
wx = ifft([X; zeros(FFTnum-size(X,1),size(X,2),size(X,3))]);
...
function [Xfull,defInconverter,f,t] = DGT_factorAlg(x,win,shift,FFTnum,in,defInconverter)
% Peter L. Sondergaard
...
function x = invDGT_factorAlg(X,win,shift,FFTnum,in,defInconverter)
% Peter L. Sondergaard
% Efficient algorithms for the discrete Gabor transform with a long FIR window
% Journal of Fourier Analysis and Applications, vol.18, pp.456-470, 2012.
...
% Phase manipulation
% ------------------------------------------------------------
function defConverter = calculateDefInConverter(X,shift,FFTnum)
f = (0:size(X,1)-1)';
t = (0:size(X,2)-1) * shift;
idx = mod(f*t,FFTnum);
defConverter = exp(2i*pi*idx/FFTnum);
